...
- **Upstream `latest` breakage**: nothing is pinned any more (`new-next.sh` v3.0.0 removed `NEXT_VERSION`, `BETTER_AUTH_VERSION`, and `kysely@^0.28.5`), so an upstream release is always a live suspect when a fresh scaffold won't build. Two known shapes: a Better Auth plugin moving out of the barrel into its own `@better-auth/*` package (that is where `apiKey` went — check `npm view @better-auth/<plugin> version` before concluding it was removed), and `@better-auth/cli` lagging the runtime far enough to omit a plugin's tables from `migrations/current.sql`. Resist re-adding a pin as the first move: the previous `next` pin outlived the preview-SWC bug it was written for and then broke `eslint.config.mjs` and `--turbopack` on its own. Verify the actual registry state (`npm view <pkg> dist-tags`) before believing a remembered breakage is still true.
